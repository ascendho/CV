...
\vspace*{-1.52cm}
\begin{center}
  {\zihao{2}\bfseries Ascend Ho}\\[0.4em]
...
\begin{itemize}
  ...
  \item \textbf{项目介绍：}针对垂直领域知识分散、检索效率低下的问题，设计并实现了一套从非结构化文本到可交互知识图谱的完整流水线。抽取阶段，通过\projkeywordzh{实体识别}（NER）与\projkeywordzh{关系抽取}（RE）联合模型从文档中提取三元组，并结合\projkeywordzh{共指消解}与\projkeywordzh{实体对齐}对抽取结果进行去重与融合。图谱构建阶段，将三元组写入 Neo4j 并建立全文索引，支持 Cypher 模板化查询与自然语言查询的双通道检索。前端基于 React 实现\projkeywordzh{力导向图}可视化，支持节点展开、路径高亮和子图导出。问答模块通过 \projkeyword{Text-to-Cypher} 将用户自然语言问题翻译为图查询语句，从图谱中检索答案并附带推理路径。
